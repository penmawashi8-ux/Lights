...
import { LEVEL_SPECS, generateLevelBoard } from "@/lib/levels";
...
interface AdventureGameScreenProps {
  levelNum: number;
  onComplete: (levelNum: number) => void;
  onBack: () => void;
}
...
export default function AdventureGameScreen({
  ...
  // Reset board when levelNum changes
  useEffect(() => {
    setBoard(generateLevelBoard(levelIndex, spec));
    setMoves(0);
    setSolved(false);
  ...
  const handleCellClick = useCallback(
    (row: number, col: number) => {
      if (solved) return;
      setBoard((prev: boolean[][]) => {
        const next = prev.map((r: boolean[]) => [...r]);
        const affected = getAffectedCells(row, col, spec.size, spec.pattern);
  ...
  useEffect(() => {
    if (!solved && isSolved(board)) {
      ...
      progress.add(levelNum);
      saveProgress(progress);

      const t = setTimeout(() => setCelebrating(false), 600);
      return () => clearTimeout(t);
    }
  }, [board, solved, levelNum]);

  const handleNextLevel = useCallback(() => {
    if (levelNum < 100) {
      onComplete(levelNum); // parent increments to get next level
    }
  }, [levelNum, onComplete]);

  const handleGoToSelect = useCallback(() => {
    onBack(); // back to level select
  }, [onBack]);
  ...
  return (
    <div className="w-full max-w-sm mx-auto space-y-3 fade-in-up">
      ...
      <div className="bg-gradient-to-r from-green-900/80 to-green-800/60 rounded-2xl px-4 py-3 border border-green-700/40 shadow text-center">
        <p className="text-yellow-200 font-black text-2xl leading-tight tracking-wide drop-shadow-md">
          ステージ {levelNum} / 100
        </p>
        <p className="text-green-300 text-xs mt-0.5">
          {PATTERN_LABELS[spec.pattern]}
          <span className="mx-2 text-green-600">|</span>
          <span className={difficultyColor}>{DIFFICULTY_LABELS[spec.difficulty]}</span>
          <span className="mx-2 text-green-600">|</span>
          {spec.size}×{spec.size}
        </p>
      </div>
        ...
        <div className="bg-gradient-to-r from-yellow-400 to-amber-400 border-2 border-yellow-300/80 rounded-2xl p-3 text-center pop-in shadow-lg shadow-yellow-500/30">
          ...
          <div className="flex gap-2 mt-3 justify-center">
            <button
              ...
              className="px-4 py-2 bg-gradient-to-b from-green-500 to-green-700 hover:from-green-400 hover:to-green-600 disabled:opacity-40 disabled:cursor-not-allowed active:scale-95 text-white font-black text-sm rounded-xl transition-all duration-150 shadow-md border-b-2 border-green-900/60"
            >
              次のステージ ▶
            </button>
            <button
              onClick={handleGoToSelect}
              className="px-4 py-2 bg-gradient-to-b from-amber-500 to-amber-700 hover:from-amber-400 hover:to-amber-600 active:scale-95 text-white font-black text-sm rounded-xl transition-all duration-150 shadow-md border-b-2 border-amber-900/60"
            >
              ステージ選択へ
            </button>
          </div>
      ...
      <div className="relative flex items-center justify-center py-1">
        ...
        />
        <div
          className="absolute -right-3 top-1/2 -translate-y-1/2 select-none pointer-events-none float-anim"
          style={{ animationDelay: "0.8s" }}
        >
          <img
            src="/chars/raccoon.png"
            alt=""
            className="w-16 h-16 object-contain"
            draggable={false}
          />
      ...
      <div className="flex items-end gap-2">
        ...
        <button
          ...
          disabled={solved}
          className="flex-1 py-3.5 bg-gradient-to-b from-green-500 to-green-700 hover:from-green-400 hover:to-green-600 disabled:opacity-50 disabled:cursor-not-allowed active:scale-95 text-white font-black text-base rounded-2xl transition-all duration-150 shadow-lg shadow-green-900/50 border-b-4 border-green-800/60 flex items-center justify-center gap-2"
        >
          リセット <span>🔄</span>
        </button>
        <div
